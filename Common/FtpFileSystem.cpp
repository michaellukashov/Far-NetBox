...
{
  std::wstring Command = FORMAT("CWD %s", (Directory));
  FFileZillaIntf->CustomCommand(Command.c_str());
  ...
  if (FCurrentDirectory.IsEmpty())
  {
    ...
    try
    {
      ...
      if ((Code == 257) &&
          (Response->GetCount() == 1))
      {
        std::wstring Path = Response->Text;
...
void TFTPFileSystem::DoReadDirectory(TRemoteFileList * FileList)
{
  ...
  std::wstring Directory = AbsolutePath(FileList->Directory, false);
    ...
    try
    {
      ...
      else if (FListAll == asAuto)
      {
        ...
        if ((FileList->GetCount() == 0) ||
            ((FileList->GetCount() == 1) && FileList->Files[0]->IsParentDirectory))
        {
          Repeat = true;
          FListAll = asOff;
...
//---------------------------------------------------------------------------
TDateTime TFTPFileSystem::ConvertRemoteTimestamp(time_t Time, bool HasTime)
{
  TDateTime Result;
  tm * Tm = localtime(&Time);
  if (Tm != NULL)
