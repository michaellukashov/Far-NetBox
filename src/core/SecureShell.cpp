...
void TSecureShell::StoreToConfig(TSessionData * Data, Config * cfg, bool Simple)
{
  ...
  ASCOPY(cfg->host, W2MB(Data->GetHostName(), Data->GetCodePageAsNumber());
         // cfg->host = ::StrNew(W2MB(Data->GetHostName().c_str(), Data->GetCodePageAsNumber()).c_str());
         ASCOPY(cfg->username, W2MB(Data->GetUserName(), Data->GetCodePageAsNumber()).c_str());
         // cfg->username = ::StrNew(W2MB(Data->GetUserName().c_str(), Data->GetCodePageAsNumber())).c_str();
         ...
         // cfg->ssh_rekey_data = ::StrNew(W2MB(Data->GetRekeyData().c_str(), Data->GetCodePageAsNumber())).c_str();
  ...
  for (int k = 0; k < KEX_COUNT; k++)
{
  ...
  // cfg->keyfile.path = ::StrNew(W2MB(SPublicKeyFile.c_str(), Data->GetCodePageAsNumber())).c_str();
  ...
  // cfg->proxy_host = ::StrNew(W2MB(Data->GetProxyHost().c_str(), Data->GetCodePageAsNumber())).c_str();
  cfg->proxy_port = Data->GetProxyPort();
  ASCOPY(cfg->proxy_username, W2MB(Data->GetProxyUsername().c_str(), Data->GetCodePageAsNumber()));
  // cfg->proxy_username = ::StrNew(W2MB(Data->GetProxyUsername().c_str(), Data->GetCodePageAsNumber())).c_str();
  ASCOPY(cfg->proxy_password, W2MB(Data->GetProxyPassword().c_str(), Data->GetCodePageAsNumber()));
  // cfg->proxy_password = ::StrNew(W2MB(Data->GetProxyPassword().c_str(), Data->GetCodePageAsNumber())).c_str();
  if (Data->GetProxyMethod() == pmCmd)
{
  ASCOPY(cfg->proxy_telnet_command, W2MB(Data->GetProxyLocalCommand().c_str(), Data->GetCodePageAsNumber()));
    // cfg->proxy_telnet_command = ::StrNew(W2MB(Data->GetProxyLocalCommand().c_str(), Data->GetCodePageAsNumber())).c_str();
  }
  else
  {
    ASCOPY(cfg->proxy_telnet_command, W2MB(Data->GetProxyTelnetCommand().c_str(), Data->GetCodePageAsNumber()));
    // cfg->proxy_telnet_command = ::StrNew(W2MB(Data->GetProxyTelnetCommand().c_str(), Data->GetCodePageAsNumber())).c_str();
  }
  ...
  if (!Data->GetTunnelPortFwd().IsEmpty())
{
  assert(!Simple);
    ...
    // cfg->portfwd = ::StrNew(W2MB(Data->GetTunnelPortFwd().c_str(), Data->GetCodePageAsNumber()).c_str());
  ...
  else
  {
    ...
    if (Data->GetFSProtocol() == fsSCPonly)
    {
      ...
      {
        // ASCOPY(cfg->remote_cmd_ptr, W2MB(Data->GetShell().c_str(), Data->GetCodePageAsNumber()));
        cfg->remote_cmd_ptr = ::StrNew(W2MB(Data->GetShell().c_str(), Data->GetCodePageAsNumber()).c_str());
      }
    }
    else
    {
      ...
      {
        cfg->ssh_subsys = FALSE;
        // cfg->remote_cmd_ptr = (char *)W2MB(Data->GetSftpServer().c_str(), Data->GetCodePageAsNumber()).c_str();
        cfg->remote_cmd_ptr = ::StrNew(W2MB(Data->GetSftpServer().c_str(), Data->GetCodePageAsNumber()).c_str());
      }
      ...
      if (Data->GetFSProtocol() != fsSFTPonly)
      {
        ...
        {
          cfg->remote_cmd_ptr2 = ::StrNew(W2MB(Data->GetShell().c_str(), Data->GetCodePageAsNumber()).c_str());
        }
      }
...
void TSecureShell::Open()
{
  ...
  const char * InitError = FBackend->init(this, &FBackendHandle, FConfig,
                                          const_cast<char *>(W2MB(FSessionData->GetHostName().c_str(), FSessionData->GetCodePageAsNumber()).c_str()), FSessionData->GetPortNumber(), &RealHost, 0,
                                          FConfig->tcp_keepalives);
  sfree(RealHost);
  if (InitError)
  {
    PuttyFatalError(MB2W(InitError, FSessionData->GetCodePageAsNumber()));
  }
...
#define SERVER_VERSION_MSG L"Server version: "
  ...
  if (Str.Pos(UnicodeString(SERVER_VERSION_MSG)) == 0)
  {
    ...
    // int pos = FSessionInfo.SshVersionString.Pos('-');
    // if (pos >= 0)
...
void TSecureShell::CWrite(const char * Data, size_t Length)
{
  ...
  FCWriteTemp += DeleteChar(UnicodeString(MB2W(std::string(Data, Length).c_str(), FSessionData->GetCodePageAsNumber())), '\r');
...
void TSecureShell::FromBackend(bool IsStdErr, const char * Data, size_t Length)
{
  ...
  // DEBUG_PRINTF(L"IsStdErr = %d, Length = %d, Data = '%s'", IsStdErr, Length, MB2W(Data, FSessionData->GetCodePageAsNumber()).c_str());
  ...
  if (IsStdErr)
  {
    ...
    AddStdError(MB2W(Str.c_str(), FSessionData->GetCodePageAsNumber()));
  }
  else
  {
...
UnicodeString TSecureShell::ReceiveLine()
{
  ...
  do
  {
    ...
    if (PendLen > 0)
    {
      ...
      // DEBUG_PRINTF(L"PendLen = %d, Index = %d, EOL = %d, Pending = %s", PendLen, Index, EOL, MB2W(Pending, FSessionData->GetCodePageAsNumber()).c_str());
  ...
  UnicodeString LineW = ::TrimRight(MB2W(Line.c_str(), FSessionData->GetCodePageAsNumber()));
...
{
  CheckConnection();
  std::string str = W2MB(Str.c_str(), FSessionData->GetCodePageAsNumber());
  ...
  const TPuttyTranslation * Translation, size_t Count, UnicodeString & Message)
{
  ...
  for (size_t Index = 0; Index < Count; Index++)
  {
    ...
    // DEBUG_PRINTF(L"Original = %s", MB2W(Original, FSessionData->GetCodePageAsNumber()).c_str());
    const char * Div = strchr(Original, '%');
    if (Div == NULL)
    {
      if (strcmp(W2MB(Message.c_str(), FSessionData->GetCodePageAsNumber()).c_str(), Original) == 0)
      {
    ...
    else
    {
      ...
      if ((static_cast<size_t>(Message.Length()) >= OriginalLen - 1) &&
          (strncmp(W2MB(Message.c_str(), FSessionData->GetCodePageAsNumber()).c_str(), Original, PrefixLen) == 0) &&
          (strncmp(W2MB(Message.c_str(), FSessionData->GetCodePageAsNumber()).c_str() + Message.Length() - SuffixLen, Div + 1, SuffixLen) == 0))
      {
        Message = FMTLOAD(Translation[Index].Translation,
                          ::TrimRight(Message.SubString(PrefixLen, Message.Length() - PrefixLen - SuffixLen)).c_str());
        // DEBUG_PRINTF(L"Message = %s", Message.c_str());
        Result = static_cast<int>(Index);
        break;
      }
...
void TSecureShell::UpdatePortFwdSocket(SOCKET value, bool Startup)
{
  ...
  SocketEventSelect(value, FSocketEvent, Startup);
  // DEBUG_PRINTF(L"Startup = %d, FPortFwdSockets.size = %d", Startup, FPortFwdSockets.Length());
  if (Startup)
  {
                                 ...
                                 const UnicodeString KeyType, const UnicodeString KeyStr, const UnicodeString Fingerprint)
{
  ...
  if (!Result)
  {
    ...
    if (retrieve_host_key(W2MB(host.c_str(), FSessionData->GetCodePageAsNumber()).c_str(), Port, W2MB(KeyType.c_str(), FSessionData->GetCodePageAsNumber()).c_str(),
                          const_cast<char *>(StoredKeys2.c_str()), static_cast<int>(StoredKeys2.Length())) == 0)
#else
    if (verify_host_key(W2MB(host.c_str(), FSessionData->GetCodePageAsNumber()).c_str(), Port, W2MB(KeyType.c_str(), FSessionData->GetCodePageAsNumber()).c_str(),
                        (char *)StoredKeys2.c_str()))
#endif
      // if (0)
    {
      StoredKeys = MB2W(StoredKeys2.c_str(), FSessionData->GetCodePageAsNumber()); // PackStr(StoredKeys);
  ...
  if (!Result)
  {
    ...
    else
    {
      ...
      case qaOK:
        ...
        // fall thru
      case qaYes:
        store_host_key(W2MB(host.c_str(), FSessionData->GetCodePageAsNumber()).c_str(), Port, W2MB(KeyType.c_str(), FSessionData->GetCodePageAsNumber()).c_str(),
                       const_cast<char *>(W2MB(keyStr.c_str(), FSessionData->GetCodePageAsNumber()).c_str()));
        break;
