...
void TTerminal::LogFileDone(TFileOperationProgressType * OperationProgress)
{
  ...
  if (GetLog()->GetLogging())
  {
    LogEvent(FORMAT("Transfer done: '%s' [%s]", OperationProgress->FullFileName, Int64ToStr(OperationProgress->TransferedSize)));
  }
}
  ...
  bool AllowDirs, TCalculateSizeStats * Stats)
{
  ...
  Param.Result = true;
  ProcessFiles(AFileList, foCalculateSize, nb::bind(&TTerminal::CalculateFileSize, this), &Param);
  Size = Param.Size;
  return Param.Result;
}
...
TTerminal * TTerminal::GetCommandSession()
{
  ...
  if (FCommandSession == nullptr)
  {
    ...
    try__catch
    {
      ...
      CommandSession->SetOnClose(nb::bind(&TTerminal::CommandSessionClose, this));
      ...
      try__finally
      {
        ...
        try
        {
          try__finally
          {
            ...
            if (GetLog()->GetLogging())
            {
              LogEvent(FORMAT("Copying %d files/directories to local directory "
                "\"%s\"", AFilesToCopy->GetCount(), TargetDir));
              LogEvent(CopyParam->GetLogStr());
            }
