...
__removed const int32_t SFTPMinVersion = 0;
__removed const int32_t SFTPMaxVersion = 6;
...
public:
  ...
  uint32_t GetMessageNumber() const { return nb::ToUIntPtr(FMessageNumber); }
...
private:
  ...
  static const int32_t FSendPrefixLen = 4;
...
protected:
  virtual bool InitRequest(TSFTPQueuePacket * ARequest) override
  {
    ...
    if (Result)
    {
      ...
      {
        FILE_OPERATION_LOOP_BEGIN(FTerminal, OperationProgress, folAllowSkip, FMTLOAD(READ_ERROR, FFileName), "")
        {
      ...
      if (Result)
      {
        ...
        ARequest->AddData(BlockBuf.GetData(), nb::ToUInt32(BlockBuf.GetSize()));
  ...
  TFileOperationProgressType * OperationProgress) const
{
  ...
  const uint32_t UploadPacketOverhead =
...
{
  int32_t Length = GET_32BIT(LenBuf);
...
bool TSFTPFileSystem::PeekPacket()
{
  ...
  if (Result)
  {
    int32_t Length = PacketLength(Buf, static_cast<SSH_FXP_TYPE>(-1));
  ...
  if ((Reservation < 0) || (Packet->GetCapacity() == 0))
  {
    ...
    do
    {
      ...
      else
      {
        ...
        int32_t Length = PacketLength(LenBuf, ExpectedType);
...
{
  int32_t Reservation = FPacketReservations->IndexOf(Response);
...
{
  SSH_FX_TYPE Result;
  uint32_t MessageNumber = Packet->GetMessageNumber();
    ...
    else
    {
      if (UseLink)
      {
        if (FSecureShell->GetSshImplementation() == sshiProFTPD)
        {
          ...
          int32_t ProFTPDMajorVer = ::StrToIntDef(CutToChar(ProFTPDVerStr, L'.', false), 0);
  ...
  TUploadSessionAction & Action, bool & ChildError)
{
  ...
  if (DoResume)
  {
    if (DestFileExists)
    {
      FILE_OPERATION_LOOP_BEGIN(FTerminal, OperationProgress, folAllowSkip, FMTLOAD(DELETE_ON_RESUME_ERROR, base::UnixExtractFileName(DestFullName), DestFullName), "")
      {
    ...
    // on VShell it failed
    FILE_OPERATION_LOOP_BEGIN(FTerminal, OperationProgress, folAllowSkip, FMTLOAD(RENAME_AFTER_RESUME_ERROR, base::UnixExtractFileName(OpenParams.RemoteFileName), ADestFileName), HELP_RENAME_AFTER_RESUME_ERROR)
    {
  ...
  if (SetProperties)
  {
    ...
    try
    {
      ...
      bool Resend = false;
      FILE_OPERATION_LOOP_BEGIN(FTerminal, OperationProgress, folAllowSkip, FMTLOAD(PRESERVE_TIME_PERM_ERROR3, ADestFileName), HELP_PRESERVE_TIME_PERM_ERROR)
      {
        try
        {
  ...
  bool TransferFinished, bool Request, TSFTPPacket * Packet)
{
  ...
  FILE_OPERATION_LOOP_BEGIN(FTerminal, OperationProgress, folAllowSkip, FMTLOAD(SFTP_CLOSE_FILE_ERROR, AFileName), "")
  {
    try
    {
      TSFTPPacket CloseRequest(FCodePage);
  ...
  TFileOperationProgressType * OperationProgress)
{
  ...
  {
    FILE_OPERATION_LOOP_BEGIN(FTerminal, OperationProgress, folAllowSkip,  FMTLOAD(WRITE_ERROR, ALocalFileName), "")
    {
  ...
  uint32_t /*AFlags*/, TDownloadSessionAction & Action)
{
  ...
  try__finally
  {
    ...
    FILE_OPERATION_LOOP_BEGIN(FTerminal, OperationProgress, folAllowSkip, FMTLOAD(SFTP_OPEN_FILE_ERROR, AFileName), "")
    {
    ...
    // at end of this block queue is discarded
    {
      ...
      try__finally
      {
        ...
        int64_t Offset = OperationProgress->TransferredSize + std::max(CopyParam->PartOffset, 0LL);
