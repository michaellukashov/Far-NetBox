...
constexpr intptr_t SFTP_MAX_PACKET_LEN = 1000 * 1024;
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
__removed const intptr_t SFTPMinVersion = 0;
__removed const intptr_t SFTPMaxVersion = 6;
...
#endif // #if 0
//---------------------------------------------------------------------------
constexpr uintptr_t SFTP_PACKET_ALLOC_DELTA = 256;
//---------------------------------------------------------------------------
...
public:
  ...
  explicit TSFTPPacket(TObjectClassId Kind, uintptr_t CodePage) noexcept :
    TObject(Kind)
  {
    Init(CodePage);
  }
  ...
  explicit TSFTPPacket(const TSFTPPacket &Source, uintptr_t CodePage) noexcept :
    TObject(OBJECT_CLASS_TSFTPPacket)
  {
    Init(CodePage);
    *this = Source;
  ...
  explicit TSFTPPacket(SSH_FXP_TYPE AType, uintptr_t CodePage) noexcept :
    TObject(OBJECT_CLASS_TSFTPPacket)
  {
    Init(CodePage);
    ChangeType(AType);
  ...
  explicit TSFTPPacket(const uint8_t *Source, uintptr_t Len, uintptr_t CodePage) noexcept :
    TObject(OBJECT_CLASS_TSFTPPacket)
  {
    Init(CodePage);
    FLength = Len;
  ...
  explicit TSFTPPacket(const RawByteString Source, uintptr_t CodePage) noexcept :
    TObject(OBJECT_CLASS_TSFTPPacket)
  {
    Init(CodePage);
  ...
  static SSH_FILEXFER_ATTR_TYPE AllocationSizeAttribute(intptr_t Version)
  {
...
public:
  ...
  explicit TSFTPAsynchronousQueue(TSFTPFileSystem *AFileSystem, uintptr_t CodePage) noexcept : TSFTPQueue(AFileSystem, CodePage)
  {
...
public:
  ...
  explicit TSFTPUploadQueue(TSFTPFileSystem * AFileSystem, uintptr_t ACodePage, TEncryption * Encryption) noexcept :
...
public:
  ...
  explicit TSFTPLoadFilesPropertiesQueue(TSFTPFileSystem *AFileSystem, uintptr_t CodePage) noexcept :
    TSFTPFixedLenQueue(AFileSystem, CodePage)
  {
  }
...
public:
  TSFTPBusy() = delete;
  explicit TSFTPBusy(TSFTPFileSystem *FileSystem) noexcept :
  ...
  uint32_t MaxPacketSize) const
{
  ...
  uintptr_t CPSRounded = TEncryption::RoundToBlock(OperationProgress->CPS());
  ...
  if (MaxPacketSizeValid)
  {
    ...
    else
    {
      ...
      {
        intptr_t MaxPacketSizeRounded = TEncryption::RoundToBlockDown(AMaxPacketSize);
        if (MaxPacketSizeRounded > 0)
        {
          AMaxPacketSize = static_cast<uint32_t>(MaxPacketSizeRounded);
        }
...
//---------------------------------------------------------------------------
SSH_FX_TYPE TSFTPFileSystem::GotStatusPacket(TSFTPPacket *Packet,
  SSH_FX_TYPE AllowStatus)
{
  SSH_FX_TYPE Code = Packet->GetCardinal();
...
//---------------------------------------------------------------------------
intptr_t TSFTPFileSystem::PacketLength(uint8_t *LenBuf, SSH_FXP_TYPE ExpectedType) const
{
...
bool TSFTPFileSystem::PeekPacket()
{
  ...
  if (Result)
  {
    intptr_t Length = PacketLength(Buf, static_cast<SSH_FXP_TYPE>(-1));
...
UnicodeString TSFTPFileSystem::Canonify(UnicodeString APath)
{
  ...
  if (TryParent)
  {
    ...
    {
      UnicodeString Path3 = base::UnixExtractFilePath(Path2);
      try
      {
  ...
  SSH_FX_TYPE AllowStatus)
{
  ...
  AddPathString(Packet, LocalCanonify(AFileName));
  ...
  if (Packet.GetType() == SSH_FXP_ATTRS)
  {
    ...
    if (FTerminal->IsFileEncrypted(AFileName))
    {
      AFile->SetEncrypted();
    }
  }
...
//---------------------------------------------------------------------------
void TSFTPFileSystem::DoDeleteFile(UnicodeString AFileName, SSH_FXP_TYPE Type)
{
  TSFTPPacket Packet(Type, FCodePage);
...
//---------------------------------------------------------------------------
void TSFTPFileSystem::RemoteRenameFile(UnicodeString AFileName, const TRemoteFile * /*File*/,
...
//---------------------------------------------------------------------------
void TSFTPFileSystem::RemoteCopyFile(UnicodeString AFileName, const TRemoteFile * /*AFile*/,
  UnicodeString ANewName)
{
  ...
  UnicodeString RealName = Canonify(AFileName);
  bool Encrypted = FTerminal->IsFileEncrypted(RealName);
  AddPathString(Packet, RealName);
  AddPathString(Packet, Canonify(ANewName), Encrypted);
  Packet.AddBool(false);
  SendPacketAndReceiveResponse(&Packet, &Packet, SSH_FXP_STATUS);
}
//---------------------------------------------------------------------------
void TSFTPFileSystem::RemoteCreateDirectory(UnicodeString ADirName, bool Encrypt)
{
...
//---------------------------------------------------------------------------
void TSFTPFileSystem::RemoteCreateLink(UnicodeString AFileName,
  ...
  TCalculatedChecksumEvent OnCalculatedChecksum)
{
  ...
  FTerminal->OperationStart(Progress, foCalculateChecksum, osRemote, AFileList->Count);
  try__finally
  {
  ...
  TSpaceAvailable &ASpaceAvailable)
{
  ...
  else if (DebugAlwaysTrue(FSupportsStatVfsV2))
  {
    ...
    if (Flags != 0)
    {
      AddToList(FlagStr, UnicodeString("0x") + ::IntToHex(nb::ToUIntPtr(Flags), 2), L",");
    }
    if (FlagStr.IsEmpty())
    {
      FlagStr = "none";
  ...
  TUploadSessionAction &Action, bool &ChildError)
{
  ...
  try__finally
  {
    ...
    else if (ResumeTransfer || (OpenParams.OverwriteMode == omResume))
    {
      ...
      FileSeek(static_cast<THandle>(AHandle.Handle), ResumeOffset, 0);
...
RawByteString TSFTPFileSystem::SFTPOpenRemoteFile(
  UnicodeString AFileName, SSH_FXF_TYPE OpenType, bool EncryptNewFiles, int64_t Size)
{
...
void TSFTPFileSystem::DirectorySunk(
  UnicodeString ADestFullName, const TRemoteFile *AFile, const TCopyParamType *CopyParam)
{
  ...
  try__finally
  {
    ...
    if (ResumeAllowed)
    {
      ...
      if (::SysUtulsFileExists(ApiPath(DestPartialFullName)))
      {
        ...
        {
          FTerminal->LogEvent("Resuming file transfer.");
          FileSeek(static_cast<THandle>(LocalFileHandle), ResumeOffset, 0);
          OperationProgress->AddResumed(ResumeOffset);
        }
      }
    ...
    if ((Attrs >= 0) && !ResumeTransfer)
    {
      ...
      else
      {
        ...
        ResumeAllowed = false;
        FileSeek(static_cast<THandle>(LocalFileHandle), DestFileSize, 0);
