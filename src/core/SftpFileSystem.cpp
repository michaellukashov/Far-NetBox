    ...
    while (Index < Dump.Length())
    {
      ...
      if (((C >= '0') && (C <= '9')) || ((C >= 'A') && (C <= 'Z')))
      {
        ...
        else
        {
          ...
          GetData()[GetLength()] = HexToByte(UnicodeString(reinterpret_cast<char *>(Byte)));
          Length++;
          memset(Byte, '\0', sizeof(Byte));
        }
      }
    ...
    size_t MaxPacketSize)
{
  ...
  unsigned long  esult = OperationProgress->CPS();
  ...
  if ((Reservation < 0) || (Packet->GetCapacity() == 0))
  {
    ...
    do
    {
          ...
          Packet->GetMessageNumber() != (unsigned int)FPacketNumbers[Reservation])
      {
        ...
        for (int Index = 0; Index < FPacketNumbers.Length(); Index++)
        {
