  ...
  if (FWorkFromCwd == asOn)
  {
    ...
    while (!base::UnixSamePath(Actual, ADirectory))
    {
      if (base::UnixIsChildPath(Actual, ADirectory))
      {
        ...
        Actual = base::UnixIncludeTrailingBackslash(TUnixPath::Join(Actual, SubDirectory));
      }
      else
      {
        SendCommand(L"CDUP");
...
void TFTPFileSystem::FileTransferProgress(int64_t TransferSize,
  int64_t Bytes)
{
  const TGuard Guard(FTransferStatusCriticalSection);
  ...
  uint32_t AFlags, TDownloadSessionAction & Action)
{
  ...
  {
    // ignore file list
    const TFTPFileListHelper Helper(this, nullptr, true);
  ...
  TUploadSessionAction & Action, bool & /*ChildError*/)
{
  ...
  {
    const TTouchSessionAction TouchAction(FTerminal->GetActionLog(), DestFullName, Handle.Modification);
  ...
  TRemoteFile *& AFile)
{
  ...
  try__finally
  {
    ...
    const TFTPFileListHelper Helper(this, FileList.get(), false);
...
bool TFTPFileSystem::FTPPostMessage(uint32_t Type, WPARAM wParam, LPARAM lParam)
{
  ...
  const TGuard Guard(FQueueCriticalSection);
            ...
            NoFileParams ? nullptr : &FileParams, UserData.CopyParam, UserData.Params, AutoResume))
      {
        switch (OverwriteMode)
        {
          case omOverwrite:
            ...
            {
              wcsncpy_s(FileName1, FileName1Len, TargetFileName.c_str(), TargetFileName.Length());
...
bool TFTPFileSystem::HandleReply(int32_t Command, uint32_t Reply)
{
  ...
  else
  {
    if (FTerminal->GetConfiguration()->GetActualLogProtocol() >= 1)
    {
      FTerminal->LogEvent(FORMAT("Got reply %d to the command %d", nb::ToInt32(Reply), Command));
    }
