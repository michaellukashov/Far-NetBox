...
void Download(TTerminal * Terminal, const UnicodeString FileName, int UseDefaults, bool & Browse, UnicodeString & BrowseFile)
{
  ...
  try
  {
    ...
    const UnicodeString FriendyFileName = TUnixPath::Join(UnixExtractFilePath(FileName), DisplayName);
            ...
            try
            {
              ...
              if (DataList->Count > 0)
              {
                ...
                if (!DownloadFile.IsEmpty())
                {
                  ...
                  DownloadFile = TUnixPath::Join(Session->SessionData->RemoteDirectory, DownloadFile);
