...
void CAsyncSslSocketLayer::OnReceive(int nErrorCode)
{
  if (m_bUseSSL)
  {
    ...
    // Receive data
    numread = ReceiveNext(buffer, (int)len);
    if (numread > 0)
    {
...
void CAsyncSslSocketLayer::OnSend(int nErrorCode)
{
  if (m_bUseSSL)
  {
    ...
    while (numread > 0)
    {
      ...
      numread = BIO_read(m_nbio, buffer, (int)len);
      if (numread <= 0)
      {
        m_mayTriggerWrite = true;
      }
  ...
  CFileZillaTools * tools)
{
...

  {
    ...
    if (!m_ssl_ctx)
    {
      ...
      {
        USES_CONVERSION;
        // TEMP: disable cert verify + force TLS 1.2 to bypass OpenSSL bn_div_words crash (issue #389)
        // The crash is in TLS 1.3 CertificateVerify RSA signature; TLS 1.2 avoids it
