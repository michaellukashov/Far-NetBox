			...
			if (code == 3 || code == 2)
			{
				...
				if (pData->transferfile.get)
				{
					if (pData->transferdata.bResume)
						...
						res = m_pDataFile->Open(pData->transferfile.localfile,CFile::modeCreate|CFile::modeWrite|CFile::shareDenyWrite);
				}
				else
#ifdef MPEXT
...
void CFtpControlSocket::TransferFinished(bool preserveFileTimeForUploads)
{
				...
				pData->transferfile.get)
	{
		...
		m_pTools->PreserveDownloadFileTime(
			(HANDLE)m_pDataFile->m_hFile, reinterpret_cast<void *>(pData->transferfile.UserData));
		#else
		if (pData->pFileTime)
		{
			SYSTEMTIME stime;
