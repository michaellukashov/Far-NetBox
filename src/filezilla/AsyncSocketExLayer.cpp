    ...
    for (res1 = res0; res1; res1 = res1->ai_next)
    {
      ...
      if (m_nFamily == AF_UNSPEC)
      {
        ...
        if (m_pOwnerSocket->m_pendingCallbacks.size())
          ::PostMessageW(m_pOwnerSocket->GetHelperWindowHandle(), WM_USER + 2, (WPARAM)m_pOwnerSocket->m_SocketData.nSocketIndex, 0);
      }
      ...
      if (!( res = ( SOCKET_ERROR != connect(m_pOwnerSocket->GetSocketHandle(), res1->ai_addr, (int)res1->ai_addrlen) ) )
...
int CAsyncSocketExLayer::DoLayerCallback(int nType, int32_t nParam1, intptr_t nParam2, char *str /*=0*/)
{
  if (!m_pOwnerSocket)
    return 0;
...
bool CAsyncSocketExLayer::TryNextProtocol()
{
  ...
  for (; m_nextAddr; m_nextAddr = m_nextAddr->ai_next)
  {
    ...
    if (connect(m_pOwnerSocket->GetSocketHandle(), m_nextAddr->ai_addr, (int)m_nextAddr->ai_addrlen) == SOCKET_ERROR && ::WSAGetLastError() != WSAEWOULDBLOCK)
    {
      m_pOwnerSocket->DetachHandle(m_pOwnerSocket->m_SocketData.hSocket);
      closesocket(m_pOwnerSocket->m_SocketData.hSocket);
      m_pOwnerSocket->m_SocketData.hSocket = INVALID_SOCKET;
