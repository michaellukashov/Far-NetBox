...
bool CFTP::Connect(HANDLE abortEvent, std::wstring &errorInfo)
{
    ...
    // DEBUG_PRINTF(L"CFTP::Connect: connecting to %s", url);
    ...
    // DEBUG_PRINTF(L"CFTP::Connect: end");
    return true;
}
...
bool CFTP::CheckExisting(const wchar_t *path, const ItemType type, bool &isExist, std::wstring &errorInfo)
{
    ...
    // DEBUG_PRINTF(L"CheckExisting: ftpPath = %s", ::MB2W(ftpPath.c_str()).c_str());
    ...
    // DEBUG_PRINTF(L"CheckExisting: urlCode = %d", urlCode);
    if (urlCode != CURLE_OK)
    {
        m_lastErrorCurlCode = urlCode;
        errorInfo = ::MB2W(curl_easy_strerror(urlCode));
        // DEBUG_PRINTF(L"CheckExisting: 1: urlCode = %u, errorInfo = %s", urlCode, errorInfo.c_str());
...
bool CFTP::GetList(PluginPanelItem **items, int *itemsNum, std::wstring &errorInfo)
{
    assert(items);
    assert(itemsNum);
    // DEBUG_PRINTF(L"GetList: m_CurrentDirectory = %s", m_CurrentDirectory.c_str());

    std::string ftpPath = LocalToFtpCP(m_CurrentDirectory.c_str(), true);
    ::AppendChar(ftpPath, '/');
    // DEBUG_PRINTF(L"GetList: ftpPath = %s", ::MB2W(ftpPath.c_str()).c_str());
    ...
    while ((std::wstring::npos != nextPos) || (std::wstring::npos != lastPos))
    {
        ...
        if (_strnicmp(singleString.c_str(), "total ", 6))
        {
            ...
            // DEBUG_PRINTF(L"ftpItem.Name = %s, FullText = %s", ftpItem.Name.c_str(), ftpItem.FullText.c_str());
...
bool CFTP::Rename(const wchar_t *srcPath, const wchar_t *dstPath, const ItemType type, std::wstring &errorInfo)
{
    ...
    // DEBUG_PRINTF(L"Rename: srcPath = %s, dstPath = %s, type = %u", srcPath, dstPath, type);
    CSlistURL slist;
...
bool CFTP::Delete(const wchar_t *path, const ItemType type, std::wstring &errorInfo)
{
    ...
    // DEBUG_PRINTF(L"Delete: ftpCommand = %s", ::MB2W(ftpCommand.c_str()).c_str());
    const CURLcode urlCode = m_CURL.ExecuteFtpCommand(ftpCommand.c_str());
    if (urlCode != CURLE_OK)
    {
        m_lastErrorCurlCode = urlCode;
...
bool CFTP::ParseFtpList(const char *text, FTPItem &item) const
{
    ...
    if (sscanf_s(text, "%c%s %*d %s %s %I64d %s %d %d:%d %[^\n]", &typeSymb, sizeof(typeSymb), permission, sizeof(permission), owner, sizeof(owner), group, sizeof(group), &size, monthName, sizeof(monthName), &day, &hours, &minutes, name, sizeof(name)) == 10)
    {
        ...
        // DEBUG_PRINTF(L"typeSymb = %c", typeSymb);
        if (typeSymb == 'l')
        {
