...
perl Configure VC-WIN32 enable-static-engine no-shared no-hw no-camellia no-seed no-mdc2 no-rc4 no-rc5 no-cms no-krb5 no-ec no-whirlpool no-srp no-gost no-idea no-ripemd no-des no-asn1 -Ob2 -Oi -Os -Oy -GL -GF -GS- -Gy -arch:SSE2 -DNDEBUG;BN_ASM;OPENSSL_NO_CAPIENG;NO_CHMOD;OPENSSL_NO_DGRAM;OPENSSL_NO_RIJNDAEL;DSO_WIN32
call ms\do_nasm
nmake -f ms\nt.mak
mkdir x86
