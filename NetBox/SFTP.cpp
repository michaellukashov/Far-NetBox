...
public:
	...
	inline operator LIBSSH2_SFTP_HANDLE* () const		{ return _Object; }
	inline LIBSSH2_SFTP_HANDLE* operator->() const		{ return _Object; };
...
bool CSFTP::GetList(PluginPanelItem** items, int* itemsNum, wstring& errorInfo)
{
	...
	struct SFTPItem {
		SFTPItem() : Attributes(0), Size(0) { Modified.dwLowDateTime = Modified.dwHighDateTime = LastAccess.dwLowDateTime = LastAccess.dwHighDateTime =0; }
		wstring				Name;
		DWORD				Attributes;
		FILETIME			Modified;
		FILETIME			LastAccess;
