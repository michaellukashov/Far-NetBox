...
public:
	//From IProtocol
	bool Connect(HANDLE, wstring&)	{ assert(NULL); return false; }
	void Close()					{}
	...
	const wchar_t* GetCurrentDirectory() { return _CurrentDirectory.c_str(); }
	...
	bool GetFile(const wchar_t*, const wchar_t*, const unsigned __int64, wstring&) { assert(NULL); return false; }
	bool PutFile(const wchar_t* remotePath, const wchar_t* localPath, const unsigned __int64 fileSize, wstring& errorInfo);
	bool Rename(const wchar_t*, const wchar_t*, const ItemType, wstring&) { assert(NULL); return false; }
	bool Delete(const wchar_t* path, const ItemType type, wstring& errorInfo);
	wstring GetURL(const bool) { return wstring(); }
