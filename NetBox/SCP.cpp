...
{
    return PSessionEditor(new CSessionEditorSCP(this));
}

PProtocol CSessionSCP::CreateClientInstance() const
{
    return PProtocol(new CSCP(this));
}


CSCP::CSCP(const CSession *session) :
    CFTP(session),
...
CURLcode CSCP::CURLPrepare(const char *ftpPath, const bool handleTimeout /*= true*/)
{
    CURLcode urlCode = CFTP::CURLPrepare(ftpPath, handleTimeout);
    if (urlCode == CURLE_OK)
    {
        // DEBUG_PRINTF(L"NetBox: CSCP::CURLPrepare: path = %s", ftpPath);
...
bool CSCP::TryToResolveConnectionProblem()
{
