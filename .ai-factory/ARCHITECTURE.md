...
|| [winscp-netbox-gap-analysis](references/winscp-netbox-gap-analysis.md) | Comprehensive WinSCP vs NetBox feature gap analysis: 12 actionable gaps, source structure mapping, feasibility assessment for Far plugin context |
|| [master-password-infrastructure-research](references/master-password-infrastructure-research.md) | Master password hardening: RecryptPasswords MSVC stub analysis, ChangeMasterPassword exception safety, WinSCP UX/backend comparison, thread safety, rate limiting, i18n gaps |
|| [silent-mode-ui-and-refinements-exploration](references/silent-mode-ui-and-refinements-exploration.md) | Silent mode UI and refinements: add Preferences checkbox, boAll→boOlder overwrite mode, suppress all plugin-layer confirmations, write error reports to .errors file. Full implementation audit of existing code. |