...
- **Loop counters and iterator variables:** Uppercase single letter (e.g., `I`, `J`, `K` — `for (int32_t I = 0; ...)`, `for (auto I = List.begin(); ...)`, `for (auto & I : Collection)`). Do not use `i`, `it`, `iter`, or descriptive names for iteration variables.
