...
**Branch:** N/A (work on current branch; no feature branch). MANDATORY: Execute all tasks 1-8 in dependency order. "Fast mode" does NOT permit skipping verification or tracing tasks.  
**Created:** 2026-04-25
**Refined:** 2026-04-25
...
- **Files:** `src/NetBox/WinSCPDialogs.cpp`
- **Deliverable:** Modify `TSessionDialog::Change()` to add index-comparison guards:
...
- **Human instruction:** Write the message: "Manual GUI testing required. Please launch `Far3_x64/Far.exe`, run the 8 test cases in `test-matrix.md`, and reply with PASS or FAIL for each case. Do not proceed to the Final checkpoint until all cases pass."
- **Blocked by:** Task 5.
...
- **Deliverable:** Use `grep -n "// TMP-LOG:"` across the three files above to locate all temporary traces added in Tasks 1-4 and 7. Remove every line containing `// TMP-LOG:`. Re-run `grep` to verify zero occurrences remain. Delete `tmp-logs.txt`.
...
- **Blocked by:** Task 7 (and Task 6 if human testing is complete).
...
5. Human tester confirms all 8 test cases in `test-matrix.md` pass.
