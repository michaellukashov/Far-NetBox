...
**Commit 2:** Certificate enumeration & lookup (Tasks 1.1, 1.2)
- Implement WindowsCertStore.cpp enumeration and thumbprint lookup
- Link ncrypt.lib, crypt32.lib

**Commit 3:** Private key acquisition & RAII (Tasks 1.3, 1.4)
- GetPrivateKeyHandle with CNG/CryptoAPI fallback
- RAII wrappers, error mapping

**Commit 4:** Session data changes (Tasks 2.1, 2.2, 2.3)
- Extend TSessionData, update save/load, add protocol logging

**Commit 5:** UI - picker and details dialogs (Tasks 3.1, 3.2, 3.3)
- Certificate picker, details dialog, radio buttons in session options

**Commit 6:** PIN prompt and caching (Task 3.4)
- PIN dialog, retry logic, cache management

**Commit 7:** SSH auth integration & conversion (Tasks 4.1, 4.2, 4.3)
- SecureShell branching, CreateOpenSSLEVPFromNCrypt, PIN caching wire-up

**Commit 8:** Integration & performance tests (Tasks 4.4, 5.1, 5.2)
- OpenSSH test server setup script, unit tests, benchmarks

**Commit 9:** Security validation & documentation (Tasks 5.3, 5.4)
