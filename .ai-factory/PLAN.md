...
- Direct SSH: `TSecureShell(this, ...)` → `FUI = TTerminal*` → `ProcessGUI()` checks Esc ✓
- Tunnel:    `TSecureShell(FTunnelUI, ...)` → `FUI = TTunnelUI*` → `ProcessGUI()` is NOOP ✗
...
|----------|----------------|-------------|
| SSH/SFTP/SCP (direct) | `EventSelectLoop` → `FUI=TTerminal*` | ✓ Already works |
| SSH Tunnel | `EventSelectLoop` → `FUI=TTunnelUI*` → noop | ✗ Root cause: TTunnelUI::ProcessGUI() |
| FTP | `WaitForMessages()` → `FTerminal->ProcessGUI()` | ✓ Already works |
| S3 | `S3_list_service/bucket` (sync C, no loop) | ✗ No ProcessGUI path |
| WebDAV | `ne_options2()` (sync neon, no loop) | ✗ No ProcessGUI path |
...
### Phase 1: Tunnel — Fix `TTunnelUI::ProcessGUI()` noop
  ...
  **How it works end-to-end:**
  ```
  TTerminal::InternalDoTryOpen() (line 1709: CheckForEsc before tunnel ✓)
